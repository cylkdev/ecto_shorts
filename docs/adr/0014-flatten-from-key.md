...
- The shape must be consistent across all contexts (top-level, nested subquery builders, SchemalessQuery).
...
Good, because `SchemalessQuery` pops the `source_key` directly from top-level params instead of nesting it inside `:from`.
...
- `lib/ecto_shorts/common_filters.ex` - flatten `:from` handling, remove `split_query_source/1`, simplify SchemalessQuery clause, remove dead helpers
