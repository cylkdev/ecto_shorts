...
- `EctoShorts.Adapter.QueryBuilder` (and its submodules) has been removed. Use `EctoShorts.DynamicBuilders` and `EctoShorts.Generator` instead.
...
- Added `EctoShorts.DynamicBuilders` which converts filter parameter maps into `Ecto.Query.DynamicExpr` values. Filtering behaviour is customizable through a pluggable adapter.
