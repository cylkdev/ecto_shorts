...
- `EctoShorts.Adapter.QueryBuilder` (and its submodules) has been removed. Use `EctoShorts.DynamicExpressions` and `EctoShorts.Generator` instead.
...
- Added `EctoShorts.DynamicExpressions` which converts filter parameter maps into `Ecto.Query.DynamicExpr` values. Filtering behaviour is customizable through a pluggable adapter.
