...
* Every BehaviourSpecDoc must define one concrete visible boundary at a time.
...
* Every BehaviourSpecDoc must define every technical term in plain language when it first appears.
* Every behaviour statement must describe observable behaviour at the chosen visible boundary.
...
1. Write `Request Restated`, `Scope Boundaries`, and `Visible Boundary` so a complete beginner can see exactly what behaviour is being specified and where it will be observed.
...
5. Write `Behaviour Specification` in the clearest proof-ready form for this repository. Use feature-file style when that is the best fit, and use repository-native test or scenario style when that is the real proof shape.
...
Use the shared `Progress`, `Current State Snapshot`, `Concrete Steps`, `Surprises & Discoveries`, `Decision Log`, `Open Questions / Blockers`, `Next Handoff`, `Outcomes & Retrospective`, and `Change Log` sections to keep the document restartable. These sections let the BehaviourSpecDoc stay open as background context while another document owns a different question.
...
Use this skeleton when you create a new BehaviourSpecDoc. Keep it complete enough that a complete beginner can continue from the document alone.
    ...
    Examples:
        ...
        **Open.** The behaviour source and visible boundary are known, and the proof-ready specification is still being drafted.
    ...
    **Legend**
    ...
    Name the visible boundary where the behaviour will be observed and proved.
