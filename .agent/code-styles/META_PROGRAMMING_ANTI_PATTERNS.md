# Meta-programming (macro) anti-patterns

This document outlines common anti-patterns when using macros (`defmacro`) and `use`-based code generation. Macros are powerful, but they can also:

- slow down compilation (because code is expanded and compiled repeatedly)
- create fragile coupling between modules (compile-time dependency graphs)
- make code harder to read (because behaviour is injected indirectly)
- break incremental recompilation (when dependencies are bypassed)
