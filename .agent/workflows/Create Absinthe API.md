...
## Parallel Work After Naming Is Fixed

One coordinator must own the resource name, root field names, module names, and target file paths first. Do not fan out file creation until those names are stable.

After the naming decisions are fixed, worker passes may create or revise the query, mutation, subscription, type, and resolver modules for that resource in parallel because those files are independent. Collect those results before you wire the root schema, shared middleware, or cross-resource imports.

Keep the root `schema.ex` update as a single-writer step. It is the collection point where imports, root fields, and shared middleware must be checked together.
