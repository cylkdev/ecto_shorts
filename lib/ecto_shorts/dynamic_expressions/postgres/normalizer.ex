defmodule EctoShorts.DynamicExpressions.Postgres.Normalizer do
  @moduledoc """
  ...
  before expression building begins. The caller (`DynamicExpressions.Adapters.Postgres`)
  ...
  @logger_prefix "EctoShorts.DynamicExpressions.Postgres.Normalizer"
  ...
  def normalize_keyword_params(source, {op, inner_term}, acc, opts) when op in @arithmetic_value_operators do
    [normalize_value_node(source, {op, inner_term}, opts) | acc]
  end

  def normalize_keyword_params(source, {wrapper, inner_term}, acc, opts) when wrapper in @datetime_wrappers do
  ...
  def normalize_value_node(source, {op, term}, opts) when op in @arithmetic_value_operators do
    case term do
      [left, right] ->
        {op, {normalize_value_node(source, left, opts), normalize_value_node(source, right, opts)}}
  ...
  def normalize_datetime_wrapper_payload(source, term, opts) when is_map(term) and not is_struct(term) do
    term
    |> Map.to_list()
