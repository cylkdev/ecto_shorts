defmodule EctoShorts.Adapter.DynamicExpression do
  @moduledoc """
  ...
  * `EctoShorts.DynamicExpressions.Postgres` - Postgres-specific expression
  ...
  The active adapter is resolved in order:
    ...
    2. `EctoShorts.Config.dynamic_adapter/0` (configured in application env).
  ...
  Then configure it:
      ...
      # config/config.exs
      config :ecto_shorts, dynamic_adapter: MyApp.CustomAdapter

  Or pass it at runtime:

      EctoShorts.Actions.all(Post, %{published: true}, dynamic_adapter: MyApp.CustomAdapter)
