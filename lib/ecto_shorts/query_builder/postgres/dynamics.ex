defmodule EctoShorts.QueryBuilder.Postgres.Dynamics do
  ...
  def build_dynamic(source, bind_select, args) do
    ...
    if is_map(args) or is_list(args) do
      ...
      reduce_dynamic(source, nil, bind_select, args)
    end
  end
...
#   def convert_params_to_dynamic(schema, dynamic, bind_select, params) do
#     reduce_dynamic(schema, dynamic, bind_select, params)
#   end

#   defp reduce_dynamic(schema, dynamic, bind_select, {bool_op, values})
#        when bool_op in @boolean_operators and is_list(values) do
#     right_dynamic =
#       reduce_boolean(values, bool_op, fn value ->
#         reduce_dynamic(schema, nil, bind_select, value)
#       end)

#     merge_dynamic(dynamic, :and, right_dynamic)
#   end

#   defp reduce_dynamic(schema, dynamic, bind_select, {key, value}) do
#     right_dynamic = dynamic_field_expr(schema, bind_select, key, value)
#     merge_dynamic(dynamic, :and, right_dynamic)
#   end

#   defp reduce_dynamic(schema, dynamic, bind_select, enum) when is_map(enum) do
#     Enum.reduce(enum, dynamic, fn pair, dyn ->
#       reduce_dynamic(schema, dyn, bind_select, pair)
#     end)
#   end

#   defp reduce_dynamic(schema, dynamic, bind_select, enum) when is_list(enum) do
#     Enum.reduce(enum, dynamic, fn value, dyn ->
#       reduce_dynamic(schema, dyn, bind_select, value)
#     end)
#   end

#   defp reduce_dynamic(_schema, _dynamic, _bind_select, term) do
#     raise ArgumentError,
...
#   """
#   def dynamic_field_expr(schema, bind_select, key, value) do
#     IO.inspect(binding())

#     cond do
#       key in @common_operators ->
#         CommonExpr.dynamic_field_expr(bind_select, key, value)
...
#               boolean_dynamic(schema, bind_select, key, {operator, value})
#             else
#               build_dynamic(schema, bind_select, key, {operator, value})
#             end
...
#   defp boolean_dynamic(schema, bind_select, key, {bool_op, values})
#        when bool_op in @boolean_operators and is_list(values) do
#     reduce_boolean(values, bool_op, fn value ->
...
#         build_dynamic(schema, bind_select, key, value)
#       end
#     end)
#   end

#   defp build_dynamic(schema, bind_select, key, {operator, value}) do
#     case schema_field_type(schema, key) do
#       {:array, _} -> ArrayExpr.dynamic_field_expr(bind_select, key, {operator, value})
#       _ -> ScalarExpr.dynamic_field_expr(bind_select, key, {operator, value})
#     end
#   end
