  ...
  def list_semantic_specs(context, binding_head_ast, target_binding_var, binding_body_asts) do
    ...
    [
      %ClauseSpec{
        ...
        guard: list_guard,
        body: AST.dynamic_ast(binding_body_asts, quote(do: unquote(field_ast) not in ^unquote(values_var)))
      },
      %ClauseSpec{
        binding_head: binding_head_ast,
        key: key_var,
        head: quote(do: {:not, {:!=, unquote(values_var)}}),
        guard: list_guard,
        body: AST.dynamic_ast(binding_body_asts, quote(do: unquote(field_ast) in ^unquote(values_var)))
      },
      %ClauseSpec{
        binding_head: binding_head_ast,
        key: key_var,
