# Task Group 05 Validation Report
...
- `rg` check for removed names (`Dynamics.Expression`, `Dynamics.Expressions.Postgres`, `ClauseEmitter`, `ClauseAdapter`, `QueryBuilder.ParamPreprocessor`) in `lib/` and `test/` (no hits)
