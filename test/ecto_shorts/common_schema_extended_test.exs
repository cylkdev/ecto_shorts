defmodule EctoShorts.CommonSchemaExtendedTest do
  ...
  describe "get_schema_source/1" do
    test "returns source tuple from a changeset" do
      changeset = Post.changeset(%Post{}, %{})
      assert CommonSchema.get_schema_source(changeset) === {"posts", Post}
    end

    test "returns nil for an unrecognized source" do
      assert CommonSchema.get_schema_source(123) === nil
    end
  end

  describe "get_schema_prefix/1" do
    test "returns prefix from a changeset" do
      alias EctoShorts.Schema.PostHasSchemaPrefix

      changeset = Changeset.change(%PostHasSchemaPrefix{})
