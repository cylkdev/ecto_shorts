defmodule EctoShorts.CommonFilters.SchemalessDatetimeWrappersTest do
  ...
  describe "convert_params_to_filter/3 datetime wrappers (schemaless)" do
    test "matches records using datetime_add before comparison" do
      expected = from(p in "posts", where: p.inserted_at >= datetime_add(p.inserted_at, ^1, "day"))
    ...
    test "excludes records using negated datetime_add comparison" do
      expected = from(p in "posts", where: not (p.inserted_at >= datetime_add(p.inserted_at, ^1, "day")))
