defmodule EctoShorts.CommonFilters.SchemalessBooleanCompositionTest do
  ...
  describe "convert_params_to_filter/3 :and shapes (schemaless)" do
    ...
    test ":and map with a range produces two where conditions AND-joined" do
      ...
      actual = CommonFilters.convert_params_to_filter("posts", %{and: %{views: [>: 10, <: 20]}}, [])
  ...
  describe "convert_params_to_filter/3 :or shapes (schemaless)" do
    ...
    test ":or is equivalent to :or_where for a single field" do
      via_or_where = CommonFilters.convert_params_to_filter("posts", %{or_where: %{views: 15}}, [])
  ...
  describe "convert_params_to_filter/3 list-of-maps / list-of-keyword-lists (schemaless)" do
    test ":and with a list of maps applies each map as a separate where clause" do
      ...
      actual = CommonFilters.convert_params_to_filter("posts", %{and: [%{views: 5}, %{published: true}]}, [])
    ...
    test ":or with a list of maps applies each map as a separate or_where clause" do
      ...
      actual = CommonFilters.convert_params_to_filter("posts", %{or: [%{views: 5}, %{published: true}]}, [])
    ...
    test ":or_where with a list of maps applies each map as a separate or_where clause" do
      ...
      actual =
        CommonFilters.convert_params_to_filter("posts", %{or_where: [%{views: 5}, %{published: true}]}, [])
  ...
  describe "convert_params_to_filter/3 keyword list compositions (schemaless)" do
    ...
    test "multiple where: and multiple or_where: all applied in order" do
      ...
      actual =
        CommonFilters.convert_params_to_filter(
          "posts",
          [where: %{published: true}, where: %{views: 0}, or_where: %{title: "A"}, or_where: %{title: "B"}],
          []
        )
