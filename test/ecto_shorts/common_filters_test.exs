defmodule EctoShorts.CommonFiltersTest do
  ...
  describe "convert_params_to_filter/1" do
    ...
    test "invalid binding params logs error and returns query unchanged" do
      ...
      assert log =~ "Expected binding params for :as to be a map or keyword list"
    ...
    test "binding selector with non-map params logs error and returns query unchanged" do
      ...
      assert log =~ "Expected nested params for :as :post to be a map or keyword list"
      assert_received {:q2, q2}
      assert q2 == q
    end

    test "unexpected filter param logs error and returns query unchanged" do
      q = from(p in Post)
      ...
      assert log =~ "Expected a query field for schema"
      assert_received {:q2, q2}
      assert q2 == q
    end
    ...
    test "supports negated explicit operator" do
      expected = from(p in Post, where: p.published != ^true)
      q = Post
      q2 = CommonFilters.convert_params_to_filter(q, %{published: %{not: %{==: true}}})
    ...
    test "supports explicit NOT IN operator for scalar fields" do
      ...
      q = Post
      q2 = CommonFilters.convert_params_to_filter(q, %{published: %{not: %{in: [true, false]}}})
    ...
    test "supports negated LIKE operator for scalar fields with list RHS (NOT LIKE ANY)" do
      ...
      q2 =
        CommonFilters.convert_params_to_filter(q, %{title: %{not: %{like: ["hello", "world"]}}})
    ...
    test "supports negated ILIKE operator for scalar fields with list RHS (NOT ILIKE ANY)" do
      ...
      q2 =
        CommonFilters.convert_params_to_filter(q, %{title: %{not: %{ilike: ["hello", "world"]}}})
    ...
    test "coerces not == with list RHS to NOT IN for scalar fields" do
      ...
      q2 = CommonFilters.convert_params_to_filter(q, %{published: %{not: %{==: [true, false]}}})
    ...
    test "supports LIKE operator for array fields (casts to text)" do
      ...
      q = Post
      q2 = CommonFilters.convert_params_to_filter(q, %{tags: %{like: "elixir"}})
    ...
    test "supports ILIKE operator for array fields (casts to text)" do
      ...
      q = Post
      q2 = CommonFilters.convert_params_to_filter(q, %{tags: %{ilike: "elixir"}})
    ...
    test "supports negated LIKE operator for array fields (casts to text)" do
      ...
      q = Post
      q2 = CommonFilters.convert_params_to_filter(q, %{tags: %{not: %{like: "elixir"}}})
    ...
    test "supports negated ILIKE operator for array fields (casts to text)" do
      ...
      q = Post
      q2 = CommonFilters.convert_params_to_filter(q, %{tags: %{not: %{ilike: "elixir"}}})
    ...
    test "supports negated LIKE operator for array fields with list RHS (NOT EXISTS ... LIKE ANY)" do
      ...
      q2 =
        CommonFilters.convert_params_to_filter(q, %{tags: %{not: %{like: ["elixir", "erlang"]}}})
    ...
    test "supports negated ILIKE operator for array fields with list RHS (NOT EXISTS ... ILIKE ANY)" do
      ...
      q2 =
        CommonFilters.convert_params_to_filter(q, %{tags: %{not: %{ilike: ["elixir", "erlang"]}}})
    ...
    test "array field supports NOT overlap via not in with list RHS" do
      ...
      q = Post
      q2 = CommonFilters.convert_params_to_filter(q, %{tags: %{not: %{in: ["elixir"]}}})
    ...
    test "array field supports contains-all via in: %{all: list}" do
      ...
      q = Post
      q2 = CommonFilters.convert_params_to_filter(q, %{tags: %{in: %{all: ["elixir", "erlang"]}}})
    ...
    test "supports boolean :or operator for multiple comparisons on same field" do
      ...
      q2 =
        CommonFilters.convert_params_to_filter(Post, %{published: %{or: [==: true, ==: false]}})
    ...
    test "invalid filter params container logs error and returns query unchanged" do
      ...
      assert log =~ "Expected :where to be a map or keyword list, got: \"bad\""
    ...
    test "invalid filter format logs error and returns query unchanged" do
      ...
      assert log =~
               "Expected nested params for :as :custom_alias to be a map or keyword list, got: \"bad\""
