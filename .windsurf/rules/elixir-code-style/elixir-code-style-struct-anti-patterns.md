...
description: Struct modules mix shape and defaults: defstruct includes default values (especially runtime expressions) while also exposing new/1 that builds structs. Two sources of truth appear (defstruct defaults vs constructor defaults). Prefer defstruct for field names only and merge defaults in one place.
---

## Elixir Code Style Rule: Keep `defstruct` as the schema, and put defaults in one default source
