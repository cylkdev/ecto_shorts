...
description: One local name is rebound to represent different intermediate meanings in the same clause (result = ...; result = result + ...). Reading later lines requires tracking which version is in scope. Use distinct names per calculation stage (excluding reduce-style accumulators).
---
