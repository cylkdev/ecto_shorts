...
description: case/cond includes a catch-all fallback (_ -> :ok, true -> :noop) that should be unreachable if invariants hold. Code quietly tolerates impossible states instead of failing fast. Guards/pattern matches or explicit raises would make assumptions explicit.
---
