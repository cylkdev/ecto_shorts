...
description: Multiple related structs repeat the same defstruct fields with the same meaning (e.g., :id, :color). Call sites pattern-match on those fields across variants. Extract a shared shape (embedded struct or common map) so the field is defined once.
---
