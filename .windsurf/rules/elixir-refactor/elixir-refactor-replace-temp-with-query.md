...
description: A local binding stores a pure derived value (subtotal = qty * price) and is used in multiple branches/places. The expression is side-effect free and worth naming as a function. Extract a defp query function and replace uses with calls (optionally keep a local cache).
---
