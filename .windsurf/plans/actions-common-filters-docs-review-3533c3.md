...
- [ ] Draft the implementation-ready documentation gap map for `EctoShorts.Actions`.
- [ ] Draft the implementation-ready documentation gap map for `EctoShorts.CommonFilters`.
- [ ] After approval, revise `lib/ecto_shorts/actions.ex` documentation only.
- [ ] After approval, revise `lib/ecto_shorts/common_filters.ex` documentation only.
- [ ] After approval, validate the revised docs against the targeted action and common-filter test suites.
...
- Decision: Use the repo-local `.windsurf/plans` path for this draft artifact.
  Rationale: The planning tool can write there in the current mode, while direct writes to repo `plans/` are blocked by the tool environment.
  Date/Author: 2026-03-15 / Cascade

## Outcomes & Retrospective

Not started yet. When the work is complete, this section should state whether both modules now document their live public surfaces adequately, which behaviors were explicitly covered with examples, which areas still rely on cross-reference instead of examples, and whether any contract mismatches were discovered and deferred.
...
From the repository root at `/Users/kurthogarth/Documents/GitHub/ecto_shorts`, implement the work in this order after approval.
...
Plan note (2026-03-15 21:49Z): Initial ExecPlan draft created after reading the governing rules, the two target modules, and the primary action/common-filter test inventory. The draft is stored under `.windsurf/plans` because the current tool mode cannot write directly to the repo `plans/` directory.
