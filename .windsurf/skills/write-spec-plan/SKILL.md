...
description: Guides the creation of a self-contained SpecPlan for producing behavior specifications with runnable proofs. Use this skill when you need to define and automate behavior specs before or during implementation.
---

## When to use

Use this skill when you need to write behavior specifications - example-driven scenarios that define what the system must do from a user-observable point of view. Use it when you want to define Given/When/Then scenarios, map them to runnable tests, and prove the behavior end-to-end.

## When not to use

Do **not** use this skill for implementing features directly. Use the `write-exec-plan` skill for that. Do **not** use this skill for clarifying requirements through Example Mapping without automation. Use the `write-example-map-plan` skill for that. Do **not** use this skill for behavior-preserving refactors. Use the `refactor` skill for that.
...
State what the user can do and observe. Phrase it as user-visible behavior, not internal implementation.

### 2. Research the codebase

Identify where behavior specs and tests live in this repo. Note existing tooling, test conventions, step definition patterns, and shared fixtures.
...
- **Behavior Vocabulary** - domain terms with clear definitions mapped to system data.
- **Behavior Spec Draft** - the initial spec in the target format with concrete example values.
...
### 4. Draft the behavior spec

Write scenarios using domain language. Each scenario must describe one outcome with concrete inputs and expected observable results. Cover the main behavior and the most important failure mode.
