defmodule EctoShorts.CompilerTest.PartitionedCompiled1474.Partition1 do
  @moduledoc false
  import Ecto.Query, only: [dynamic: 2]

  @type selected_binding :: {:as | :at, term()}
  @type key :: atom()
  @type value :: term()
  @type compose_res :: Ecto.Query.t() | nil

  @doc false
  def dynamic_expr({:as, binding_alias}, :first_key, value) do
    {:first_key, value}
  end

  def dynamic_expr({:as, binding_alias}, :second_key, value) do
    {:second_key, value}
  end

  def dynamic_expr({:at, 1}, :first_key, value) do
    {:first_key, value}
  end

  def dynamic_expr({:at, 1}, :second_key, value) do
    {:second_key, value}
  end

  def dynamic_expr({:at, 2}, :first_key, value) do
    {:first_key, value}
  end

  def dynamic_expr({:at, 2}, :second_key, value) do
    {:second_key, value}
  end

  def dynamic_expr({:at, 3}, :first_key, value) do
    {:first_key, value}
  end

  def dynamic_expr({:at, 3}, :second_key, value) do
    {:second_key, value}
  end

  def dynamic_expr({:at, 4}, :first_key, value) do
    {:first_key, value}
  end

  def dynamic_expr({:at, 4}, :second_key, value) do
    {:second_key, value}
  end

  def dynamic_expr({:at, 5}, :first_key, value) do
    {:first_key, value}
  end

  def dynamic_expr(_, _, _) do
    nil
  end
end
